...
int main() {
    ...
    while (t--) {
        ...
        std::vector f(k, std::vector(k, Z(0)));
        for (int i : ord) {
            for (int j : ord) {
                if (i != j && r[i] <= r[j] && b[i] <= b[j]) {
                    f[i][j] = comb.binom(r[j] - r[i] + b[j] - b[i], r[j] - r[i]);
                    for (int k : ord) {
                        if (i != k && r[i] <= r[k] && b[i] <= b[k]) {
                            if (k != j && r[k] <= r[j] && b[k] <= b[j]) {
                                f[i][j] -= f[i][k] * comb.binom(r[j] - r[k] + b[j] - b[k], r[j] - r[k]);
                            }
                        }
                    }
                }
        ...
        for (int i : ord) {
            for (int j : ord) {
                if (i != j && r[i] <= r[j] && b[i] <= b[j]) {
                    dp[j] += 2 * (dp[i] + (2 * (r[j] - r[i]) + b[j] - b[i]) * comb.binom(r[i] + b[i], r[i])) * f[i][j];
                }
            }
        }

        std::cout << dp[k - 1] / 2 / comb.binom(n + m, n) << "\n";
    }
